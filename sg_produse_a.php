...
if(isset($_POST['save']))
{
   ...
   $result = mysql_query("update $tab_produse set specs_".$_POST['l']." ='".$content."'  where id='".$_POST['id_pagina']."'") or die ("Eroare: ".mysql_error());
   $status_msg = "Pagina salvata";
}
?>
<br />
<form action="sg_index.php?pagina=sg_produse_a" method="post">
      <select style="width: 170px" name="paginaa" onchange="this.form.submit();">
      	<option value="-">--- alegeti produsul ---</option>
      <?php
     $result = mysql_query("select * from $tab_produse ORDER BY nume_en") or die ("Eroare: ".mysql_error());
      while($row = mysql_fetch_array($result))
      {
	  if ($row['id'] == $_POST['paginaa']) {
	  	echo "<option value=\"".$row['id']."\" SELECTED>".$row['nume_en']."</option>";}
		else { echo "<option value=\"".$row['id']."\" >".$row['nume_en']."</option>"; }
      }
      ?>
      </select>
</form>
...
<?php
$result1 = mysql_query("select * from $tab_produse where id='".@$_POST['paginaa']."'") or die ("Eroare: ".mysql_error());
$row1 = mysql_fetch_row($result1);
if ($_REQUEST['l']=="fr") $conti='11';
if ($_REQUEST['l']=="en") $conti='9';
if ($_REQUEST['l']=="de") $conti='10';
if ($_REQUEST['l']=="es") $conti='12';
echo $row1[$conti];
?>
</textarea>
<br/>
